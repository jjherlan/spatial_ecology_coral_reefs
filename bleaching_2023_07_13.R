...
coral_labels <- c(plob = "PLOB", 
                  poci = "POCI")

rpn_cover_coral.main <-
  rpn_cover_coral %>%
  add_column(func.group = rep(c(plob = "PLOB", 
                                poci = "POCI"), #,
                              #  ma = "MA",
...
rpn_coral_2015.ggbarplot <- ggplot(rpn_cover_coral.main, aes(x = factor(coast, x_labels), y = mean, fill = depth2)) +   
  ...
  scale_x_discrete(expand = c(0, 1)) + 
  scale_fill_manual(values = c("#FFC74E", "#82A5C0", "#ABC178")) + #
  facet_grid(factor(func.group, coral_labels) ~ depth2) + 
  #ggtitle(expression(paste(italic(" Porites "), "spp."))) +
  #geom_text(aes(label = cld, y = upper.ci), vjust = -0.5, size = 10) +
  #scale_y_log10(expression(paste("Colony Size (", cm^2, ")"), limits = c(0, 100000))) +
  labs(x = NULL) +
...
rpn_plob_2015.ggbarplot <- ggplot(rpn_cover_plob.main, aes(x = factor(coast, x_labels), y = mean, fill = depth2)) +   
  geom_bar(stat = "identity", 
           position = position_dodge(), width = 0.75, color = "black", linewidth = 0.50, alpha = 0.6) +
