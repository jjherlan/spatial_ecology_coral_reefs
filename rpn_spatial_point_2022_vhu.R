...
plot(K_none_vhu, legend = F, main = "Vaihu: Ripley's K")

#plot L with 1:1 expectation

L_none_vhu <- Lest(vhu_corals, correction = "none")

plot(L_none_vhu, legend = F, main = "Vaihu: standardized L function (standardized 1:1)")

#plot L with 0 expectation

plot(L_none_vhu, . - r ~ r, legend = F, main = "Vaihu: standardized L function (standardized 0)")
...
plot(L_iso_vhu, . - r ~ r, legend = F, main = "Vaihu: standardzied L (isotropic correction)")

#translate (toroidal) edge correction

L_trans_vhu <- Lest(vhu_corals, correction = "trans")

plot(L_trans_vhu, . - r ~ r, legend = F, main = "Vaihu: standardzied L (translate correction)")
...
plot(L_csr_vhu, . - r ~ r, shade = c("hi", "lo"), legend = F, main = "Vaihu: stand. L (Monte Carlo, CSR env)")

#plot global envelope

plot(L_csr.g_vhu, . - r ~ r, shade = c("hi", "lo"), legend = F, main = "Vaihu: stand. L (Monte Carlo, CSR env)")
...
# envelopes for vhuy distances, pointwise envelopes with a specified alpha should not
...
Ptrans_vhu <- pcf(vhu_corals, correction = "translate")
plot(Ptrans_vhu)

Penv_vhu <- envelope(vhu_corals, pcf, nsim = 99, rank = 1, correction =
                       "translate", global = FALSE)

plot(Penv_vhu, shade = c("hi", "lo"), legend = FALSE, main = "Vaihu: pair correlation function, g")
...
# can adjust the smoothing on the pair correlation function using the 'stoyan' comvhud
...
Gtrans_vhu <- Gest(vhu_corals, correction = "rs")

plot(Gtrans_vhu, legend = F)

Genv_vhu <- envelope(vhu_corals, Gest, nsim = 99, rank = 1,
                     correction = "rs", global = FALSE)

plot(Genv_vhu, shade = c("hi", "lo"), legend = FALSE, main = "Vaihu: G-function")

#nearest neighbor distance for each point

nn.dist_vhu <- nndist(vhu_corals)

plot(ecdf(nn.dist_vhu), add = T)
...
Kthomas_vhu <- kppm(vhu_corals, ~ 1, "Thomas")
...
Kthomas.env_vhu <- envelope(Kthomas_vhu, Lest, nsim = 99, rank = 1, global = F)

plot(Kthomas.env_vhu, . - r ~ r, shade = c("hi", "lo"), legend = F, main = "Vaihu: Thomas Model")
