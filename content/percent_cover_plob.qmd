...
```{r}
plob_percent_cover = read_csv('rpn_plob.csv') %>% 
...
```{r}
ggplot(plob_percent_cover, aes(x = plob_cover)) +
...
::: {.fold .s}
```{r}
plot(plob_cover ~ Site, data = plob_percent_cover)
```
:::
...
```{r}
plob_cover_mod.aov1 <- aov(plob_cover ~ Site, data = plob_percent_cover)
summary(plob_cover_mod.aov1)
```

```{r}
plob_cover_mod.lm1 <- lm(plob_cover ~ Site, data = plob_percent_cover)
```

```{r}
summary(plob_cover_mod.lm1)
```

```{r}
par(mfrow = c(2, 2))
plot(plob_cover_mod.lm1)
```

```{r}
Anova(plob_cover_mod.lm1, type = "III")
```

```{r}
plob_percent_cover$plob_cover_scaled <- transform01(plob_percent_cover$plob_cover)
```
...
```{r}
plob_cover_mod.lm2 <- lm(logit(plob_cover_scaled) ~ Site, data = plob_percent_cover) 
```

```{r}
summary(plob_cover_mod.lm2)
```

```{r}
par(mfrow = c(2, 2))
plot(plob_cover_mod.lm2)
```

```{r}
Anova(plob_cover_mod.lm2, type = "III")
```

**Generalized-linear model**

```{r}
plob_cover_mod.glm1 <- glm(plob_cover ~ Site, family = binomial, data = plob_percent_cover)
summary(plob_cover_mod.glm1)
```

```{r}
par(mfrow = c(2, 2))
plot(plob_cover_mod.glm1)
```

```{r}
Anova(plob_cover_mod.glm1, type = "III")
```
...
```{r}
summary(glht(plob_cover_mod.glm1, mcp(Site = "Tukey")), test = adjusted(type = "holm"))
```

```{r}
summary(glht(plob_cover_mod.glm1, emm(pairwise ~ Site)), test=adjusted(type="holm"))
```

```{r}
summary(emmeans(plob_cover_mod.glm1, pairwise ~ Site, adjust="tukey", mode="linear.predictor", type="Score"))
```
...
```{r}
plob_cover_mod.beta1 <- betareg(plob_cover_scaled ~ Site, data = plob_percent_cover, link = c("logit"), link.phi = NULL, type = c("ML"))
```

```{r}
summary(plob_cover_mod.beta1)
```

**Beta Regression II:** $\phi$ does vary (by **Site**)

```{r}
plob_cover_mod.beta2 <- betareg(plob_cover_scaled ~ Site | Site, data = plob_percent_cover, link = c("logit"), link.phi = NULL, type = c("ML"))
```

\#`{r} #test(pairs(emmeans(p_cover.bm2, ~ Site, mode = "link"))) #`

```{r}
plob_cover_mod.beta1.aov <- Anova(plob_cover_mod.beta1, type = 'III')
```

```{r}
plob_cover_mod.beta2.aov <- Anova(plob_cover_mod.beta2, type = 'III')
```

```{r}
plob_cover_mod.beta2.aov
```

```{r}
models <- list(
  
  'plob_cover_mod.beta1' = betareg(plob_cover_scaled ~ Site, 
                                data = plob_percent_cover, 
                                link = c("logit"), 
                                link.phi = NULL, 
                                type = c("ML")
                                ),
  
  'plob_cover_mod.beta2' = betareg(plob_cover_scaled ~ Site | Site, 
                      data = plob_percent_cover,
                      link = c("logit"), 
                      link.phi = NULL, 
                      type = c("ML")
...
```{r}
plob_cover_mod.beta1_aic <- AIC(plob_cover_mod.beta1)
plob_cover_mod.beta2_aic <- AIC(plob_cover_mod.beta2)
```

```{r}
plob_cover_mod.beta1_aic
plob_cover_mod.beta2_aic
```
...
tab.mod <- Fcn.CreateSummary.betareg(plob_cover_mod.beta1)
...
```{r}
p_cover_mod.beta2 <- betareg(plob_cover_scaled ~ Site | Site, data = plob_percent_cover, link = c("logit"), link.phi = NULL, type = c("ML"))
```

```{r}
summary(plob_cover_mod.beta2)
```
...
plob_cover.aov1 <- aov(plob_cover ~ Site, data = plob_percent_cover)
summary(plob_cover.aov1)
```
:::

::: {.fold .o}
```{r, eval = T, collapse = T, message = FALSE}
plob_cover.aov2 <- aov(plob_cover ~ Site + Error(plot_id), data = plob_percent_cover)
summary(plob_cover.aov2)
```
:::
...
```{r}
plob_cover.lme1 <- lme(plob_cover ~ Site, random = ~ 1 | plot_id, data = plob_percent_cover)
```

```{r}
anova(plob_cover.lme1)
```

```{r}
plob_cover.lme_null <- lme(plob_cover ~ 1, random = ~ 1 | plot_id, data = plob_percent_cover)
```

```{r}
lmtest::lrtest(plob_cover.lme1, plob_cover.lme_null)
```
...
```{r error = TRUE}
plob_cover.bm1 <- betareg(plob_cover_scaled ~ Site, data = plob_percent_cover)
```
...
```{r}
plob_cover.bmnull <- betareg(plob_cover_scaled ~ 1, data = plob_percent_cover)
summary(plob_cover.bmnull)
```

```{r}
plob_cover.lm1 <- lm(plob_cover_scaled ~ Site, data = plob_percent_cover)
summary(plob_cover.lm1)
```

```{r}
lmtest::lrtest(plob_cover.bm1, plob_cover.bmnull)
```

```{r}
AIC(plob_cover.bm1, plob_cover.lm1, plob_cover.bmnull)
```
...
coefs.bm1 <- coef(plob_cover.bm1)

# create vector spanning the transformed 0-1 interval

n.bm2 <- length(fitted(plob_cover.bm1))
...
```{r}
plot(resid(plob_cover.bm1) ~ fitted(plob_cover.bm1))
```
...
```{r}
plob_cover.bm2 <- betareg(plob_cover_scaled ~ Site | Site, data = plob_percent_cover)

summary(plob_cover.bm2)
```
...
```{r}
lmtest::lrtest(plob_cover.bm1, plob_cover.bm2)
lmtest::lrtest(plob_cover.bmnull, plob_cover.bm1, plob_cover.bm2)
```
...
```{r}
test(pairs(emmeans(plob_cover.bm2, ~ Site, mode = "link")))
```
...
Residual plots confirm our conclusion that `plob_cover.bm2` provides a better fit to the observed data than `plob_cover.bm1`. This is seen by the more even spread of residuals in the second plot below.

::: {.fold .s}
```{r fig.height=10}
par(mfrow = c(2, 1), oma = c(0, 0, 0, 0), mar = c(4, 4, 0.2, 0.2))
plot(residuals(plob_cover.bm1) ~ fitted(plob_cover.bm1))
plot(residuals(plob_cover.bm2) ~ fitted(plob_cover.bm2))
```
:::
...
muphi.bm2 <- unique(data.frame(
  mu = fitted(plob_cover.bm2),
  phi = predict(plob_cover.bm2, type = "precision"),
...
#  facet_wrap( ~ depth2, labeller = as_labeller(label_names), dir = "v", ncol = 1) + 
  ggtitle(expression(paste(italic(" plobllopora "), "spp."))) +
