...
# `{r, include = FALSE} # #p_cover = read_csv('LSMclassGrid.csv') %>%  # #  group_by(Site) %>% # #  mutate( # #    pland_decimal = pland*0.01 # #  ) %>% # #  mutate_at(vars(Site, cover, plot_id), factor) #`

# `{r, include = TRUE} # p_cover = read_csv('rpn_percent_cover.csv') %>%  #   group_by(Site) %>% #   mutate( #     pland_decimal = pland*0.01 #   ) %>% #   mutate_at(vars(Site, cover, plot_id), factor) #`
...
```{r}
bare_percent_cover = read_csv('rpn_bare.csv') %>% 
...
```{r}
ggplot(bare_percent_cover, aes(x = bare_cover)) +
  geom_histogram(fill = "#333399") + 
  #below here is ylabel, xlabel, and main title
  ylab("Frequency") +
  xlab(NULL) +
  ggtitle(expression("Bare Substrate Cover (%)")) +
  theme_bw() +
...
::: {.fold .s}
```{r}
plot(bare_cover ~ Site, data = bare_percent_cover)
```
:::
...
```{r}
bare_cover_mod.aov1 <- aov(bare_cover ~ Site, data = bare_percent_cover)
summary(bare_cover_mod.aov1)
```

```{r}
bare_cover_mod.lm1 <- lm(bare_cover ~ Site, data = bare_percent_cover)
```

```{r}
summary(bare_cover_mod.lm1)
```

```{r}
par(mfrow = c(2, 2))
plot(bare_cover_mod.lm1)
```

```{r}
Anova(bare_cover_mod.lm1, type = "III")
```

```{r}
bare_percent_cover$bare_cover_scaled <- transform01(bare_percent_cover$bare_cover)
```
...
```{r}
bare_cover_mod.lm2 <- lm(logit(bare_cover_scaled) ~ Site, data = bare_percent_cover) 
```

```{r}
summary(bare_cover_mod.lm2)
```

```{r}
par(mfrow = c(2, 2))
plot(bare_cover_mod.lm2)
```

```{r}
Anova(bare_cover_mod.lm2, type = "III")
```

**Generalized-linear model**

```{r}
bare_cover_mod.glm1 <- glm(bare_cover ~ Site, family = binomial, data = bare_percent_cover)
summary(bare_cover_mod.glm1)
```

```{r}
par(mfrow = c(2, 2))
plot(bare_cover_mod.glm1)
```

```{r}
Anova(bare_cover_mod.glm1, type = "III")
```

# df has subjects (S), one factor (X) w/levels (a,b,c), and continuous response (Y) library(multcomp) \# for glht, mcp ,

test=adjusted(type="holm")) \# the following also performs the equivalent contrast tests library(emmeans) \# for emm, emmeans summary(glht(m, emm(pairwise \~ X)), test=adjusted(type="holm")) \# or, using the Tukey HSD correction instead of Holm's summary(emmeans(m, pairwise \~ X, adjust="tukey", mode="linear.predictor", type="Score"))

```{r}
summary(glht(bare_cover_mod.glm1, mcp(Site = "Tukey")), test = adjusted(type = "holm"))
```

```{r}
summary(glht(bare_cover_mod.glm1, emm(pairwise ~ Site)), test=adjusted(type="holm"))
```

```{r}
summary(emmeans(bare_cover_mod.glm1, pairwise ~ Site, adjust="tukey", mode="linear.predictor", type="Score"))
```

## Methods for analyzing percent cover of poritid coral
...
```{r}
bare_cover_mod.beta1 <- betareg(bare_cover_scaled ~ Site, data = bare_percent_cover, link = c("logit"), link.phi = NULL, type = c("ML"))
```

```{r}
summary(bare_cover_mod.beta1)
```

**Beta Regression II:** $\phi$ does vary (by **Site**)

```{r}
bare_cover_mod.beta2 <- betareg(bare_cover_scaled ~ Site | Site, data = bare_percent_cover, link = c("logit"), link.phi = NULL, type = c("ML"))
```

\#`{r} #test(pairs(emmeans(p_cover.bm2, ~ Site, mode = "link"))) #`

```{r}
bare_cover_mod.beta1.aov <- Anova(bare_cover_mod.beta1, type = 'III')
```

```{r}
bare_cover_mod.beta2.aov <- Anova(bare_cover_mod.beta2, type = 'III')
```

```{r}
bare_cover_mod.beta2.aov
```

```{r}
models <- list(
  
  'bare_cover_mod.beta1' = betareg(bare_cover_scaled ~ Site, 
                                data = bare_percent_cover, 
                                link = c("logit"), 
                                link.phi = NULL, 
                                type = c("ML")
                                ),
  
  'bare_cover_mod.beta2' = betareg(bare_cover_scaled ~ Site | Site, 
                      data = bare_percent_cover,
                      link = c("logit"), 
                      link.phi = NULL, 
                      type = c("ML")
...
```{r}
bare_cover_mod.beta1_aic <- AIC(bare_cover_mod.beta1)
bare_cover_mod.beta2_aic <- AIC(bare_cover_mod.beta2)
```

```{r}
bare_cover_mod.beta1_aic
bare_cover_mod.beta2_aic
```

## Interpreting results for poritid corals
...
tab.mod <- Fcn.CreateSummary.betareg(bare_cover_mod.beta1)
...
```{r}
p_cover_mod.beta2 <- betareg(bare_cover_scaled ~ Site | Site, data = bare_percent_cover, link = c("logit"), link.phi = NULL, type = c("ML"))
```

```{r}
summary(bare_cover_mod.beta2)
```
...
bare_cover.aov1 <- aov(bare_cover ~ Site, data = bare_percent_cover)
summary(bare_cover.aov1)
```
:::

::: {.fold .o}
```{r, eval = T, collapse = T, message = FALSE}
bare_cover.aov2 <- aov(bare_cover ~ Site + Error(plot_id), data = bare_percent_cover)
summary(bare_cover.aov2)
```
:::
...
```{r}
bare_cover.lme1 <- lme(bare_cover ~ Site, random = ~ 1 | plot_id, data = bare_percent_cover)
```

```{r}
anova(bare_cover.lme1)
```

```{r}
bare_cover.lme_null <- lme(bare_cover ~ 1, random = ~ 1 | plot_id, data = bare_percent_cover)
```

```{r}
lmtest::lrtest(bare_cover.lme1, bare_cover.lme_null)
```
...
```{r error = TRUE}
bare_cover.bm1 <- betareg(bare_cover_scaled ~ Site, data = bare_percent_cover)
```
...
With this scaled data we can now successfully fit the model. And test its significance relative to a null model that assumes no effect of wave power on percent cover of poritid corals. For reference we also fit a classical ANOVA model assuming normally distributed errors using `lm`.

```{r}
bare_cover.bmnull <- betareg(bare_cover_scaled ~ 1, data = bare_percent_cover)
summary(bare_cover.bmnull)
```

```{r}
bare_cover.lm1 <- lm(bare_cover_scaled ~ Site, data = bare_percent_cover)
summary(bare_cover.lm1)
```

```{r}
lmtest::lrtest(bare_cover.bm1, bare_cover.bmnull)
```

```{r}
AIC(bare_cover.bm1, bare_cover.lm1, bare_cover.bmnull)
```

According to the likelihood-ratio test there is a significant difference between the null model and the treatment model. The AIC analysis supports this conclusion, but also highlights the improved model fit with beta regression relative to normal ANOVA (`lm1`). From this initial analysis we would tentatively conclude that using beta regression improves our ability to model the algal cover, but that there is no effect of grazer manipulation treatment.
...
coefs.bm1 <- coef(bare_cover.bm1)

# create vector spanning the transformed 0-1 interval

n.bm2 <- length(fitted(bare_cover.bm1))
...
```{r}
plot(resid(bare_cover.bm1) ~ fitted(bare_cover.bm1))
```
...
```{r}
bare_cover.bm2 <- betareg(bare_cover_scaled ~ Site | Site, data = bare_percent_cover)

summary(bare_cover.bm2)
```

```{r}
tab_model(bare_cover.bm2, CSS = css_theme("cells"))
```


From the `Coefficients` table we see that the estimate for $\mu$ in the Control treatment is `inv.logit(3.1694)` = `X` = X% coral cover. Moreover, the estimates of $\mu$ for the other two `Sites` (treatments) are each significantly higher.
...
```{r}
lmtest::lrtest(bare_cover.bm1, bare_cover.bm2)
lmtest::lrtest(bare_cover.bmnull, bare_cover.bm1, bare_cover.bm2)
```
...
It is possible to apply post-hoc tests to identify which pairwise contrasts of treatments levels are significant.

```{r}
test(pairs(emmeans(bare_cover.bm2, ~ Site, mode = "link")))
```
...
Residual plots confirm our conclusion that `bare_cover.bm2` provides a better fit to the observed data than `bare_cover.bm1`. This is seen by the more even spread of residuals in the second plot below.

::: {.fold .s}
```{r fig.height=10}
par(mfrow = c(2, 1), oma = c(0, 0, 0, 0), mar = c(4, 4, 0.2, 0.2))
plot(residuals(bare_cover.bm1) ~ fitted(bare_cover.bm1))
plot(residuals(bare_cover.bm2) ~ fitted(bare_cover.bm2))
```
:::
...
muphi.bm2 <- unique(data.frame(
  mu = fitted(bare_cover.bm2),
  phi = predict(bare_cover.bm2, type = "precision"),
