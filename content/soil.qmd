...
```{r}
soil_summary <- read.csv('soil_full.csv') %>%
  as_tibble() %>%
...
```{r}
soil.barplot <- ggplot(soil_summary, aes(x = range_mean, y = mean, fill = material)) +   
...
```{r}
soil_log3.aov <- aov(log(mbc) ~ material * depth_cat, data = soil)
```

```{r}
summary(soil_log3.aov)
```
...
```{r}
cld <- multcompLetters4(soil_log3.aov, tukey2) 
```
...
# Adding the compact letter display to the table with mean, se

#```{r}
#tukey.cld <- as.data.frame(cld$depth_cat)
#```

#```{r}
#data_summary$tukey <- tukey.cld$Letters
#```
