...
```{r}
Gtrans_se <- Gest(se_corals, rmax = 5, correction = "rs")
```
...
```{r}
Genv_se <- envelope(se_corals, rmax = 5, Gest, nsim = 99, rank = 1, correction = "rs", global = FALSE)
```
...
```{r}
plot(Kthomas.env_se, . - r ~ r, shade = c("hi", "lo"), legend = F, main = "Vaihu: Thomas Model")
```
